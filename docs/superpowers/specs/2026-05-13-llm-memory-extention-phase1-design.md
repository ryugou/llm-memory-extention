...
| rebuild の eventual consistency | rebuild 中の append は **同一 worker セッションの drain loop** で取り込まれる。staleness window は最大「現セッションの全 iteration が終わるまで」。当該 owner に対し何の rebuild も起きていない状態で append が来ない場合は、その raws は manual `wiki_rebuild` まで未反映 | UI 即時反映が要件になったら sync rebuild モードを追加 |
...
struct RebuildState {
    running: bool,
    manual_pending: Option<RebuildMode>,  // 連続 burst でも 1 つだけ保持
}
...
時刻の取り扱い: `started_at` と `raws.created_at` は同じ時計（壁時計ベースの epoch ms）を使う。具体的には `SystemTime::now()` 由来の `unix_timestamp_ms()` で統一し、`Instant`（単調時計）は耐久比較が必要な箇所には使わない。これは `started_at` を DB の `created_at` と直接比較するためで、混在すると境界条件が不安定になる。

注記: `SystemTime` は単調ではない。NTP による時刻巻き戻し等で `started_at < raws.created_at` 等の前後関係が反転するケースは理論上ありえるが、MVP では「クラウド VM の NTP に従う」前提で許容する。極端な逆転が観測されたら別途対処する。

worker は 3 つのモードで動く：
- **Append**: append-triggered。new_raws 起点。
- **Manual { concept: None }**: 全 concept を強制再合成
- **Manual { concept: Some(c) }**: 単一 concept を強制再合成

```
入力: mode (Append | Manual { concept: Option<String> })

iteration_count = 0
const MAX_ITERATIONS = 10  // 無限ループ耐性 (高頻度 append への安全弁)

loop {
    iteration_count += 1
    if iteration_count > MAX_ITERATIONS:
        log warn "drain loop hit MAX_ITERATIONS, deferring remainder to next trigger"
        metrics::rebuild_drain_capped_total.inc()
        break

    started_at = now_ms()
    起点 = MAX(wikis.last_rebuilt_at WHERE scope='personal' AND owner_id=?)
            該当なし なら 0
    new_raws = raws WHERE scope='personal' AND owner_id=?
                    AND created_at > 起点 AND created_at <= started_at

    // モード分岐: affected concepts の決定方法
    affected = match mode {
        Append => {
            if new_raws is empty: break  // drain 完了
            haiku_extract_affected(new_raws, existing_concepts)
        }
        Manual { concept: Some(c) } => {
            // 単一 concept 強制。Haiku は呼ばない (コスト削減)
            {c}
        }
        Manual { concept: None } => {
            // 全 concept 強制再合成
    ...
    if affected is empty: break  // 何もすることがない（初回 Manual で wiki 0 件など）

    // 影響 concept ごとに並列 (concurrency 上限 4):
    for concept in affected (parallel, max 4):
    ...
    // 次 iteration は Append モードに降格 (Manual は 1 iteration のみ強制)
    mode = Append
}

// drain 完了後:
state map をロックして atomic に:
  if state[owner].manual_pending != None:
      let next_mode = state[owner].manual_pending.take()
      // running は true のまま、上のループを再起動 (iteration_count はリセット)
      goto loop
  else:
...
要点:
- Manual モードでは `new_raws` が空でも affected concepts を強制再合成する（P0 修正）
- Manual { concept: Some(c) } は Haiku を呼ばずに affected = {c} で確定（コスト削減）
- 1 iteration で Manual が走ったあとは Append モードに降格し、以降は通常の drain
- MAX_ITERATIONS=10 で無限 drain を打ち切り。打ち切られた残分は次の append または manual で拾われる

### 7.1 manual `wiki_rebuild` ツールのセマンティクス

```
入力: concept? (省略時は全 concept)
要求モード: Manual { concept: Option<String> }

state map をロックして atomic に判定:

CASE A: running == false
  → state[owner].running = true
  → worker を spawn with mode=Manual { concept }
  → 戻り: { status: 'started' }

CASE B: running == true
  → state[owner].manual_pending = Some(Manual { concept })
    (既に Some(Manual{concept=None}) なら上書きしない。
     None と Some(c) の bust が起きた場合は None で merge:
     つまり concept=None が優先される。「全 concept 再合成」が
     上位集合のため安全。)
  → 戻り: { status: 'pending' }
     (現 rebuild の drain 完了後に追加 rebuild が走る)
```

manual_pending は `Option<ManualMode>` で持つ。連続 burst でも保持されるのは max 1 つの「次に走らせるべき manual モード」のみ。
...
tokio task の panic では `running` フラグは自動で戻らない。**正規パターンは JoinHandle await** とする。`tokio::task::JoinHandle::await` は `Result<T, JoinError>` を返し、`JoinError` で panic を捕捉できるため、標準ライブラリだけで完結する。

```rust
// 正規: 親 task で JoinHandle を await し、結果に関わらず必ず state を idle に戻す
let handle = tokio::spawn(rebuild_body(state.clone(), owner.clone()));
tokio::spawn(async move {
    let result = handle.await;
    coordinator.mark_idle(&owner);  // 成功・失敗・panic いずれでも実行
    match result {
        Ok(Ok(())) => {}                                   // 正常終了
        Ok(Err(e)) => tracing::error!(?owner, ?e, "rebuild error"),
        Err(join_err) if join_err.is_panic() => {
            tracing::error!(?owner, ?join_err, "rebuild worker panicked");
            metrics::rebuild_failed_total.inc();
        }
        Err(_) => {}                                       // cancellation 等
    }
});
```

参考（補助パターン）: `futures::FutureExt::catch_unwind` + `AssertUnwindSafe` でも実現可能だが、`futures` crate と `UnwindSafe` 境界の取り回しが必要。標準パターンは上記の JoinHandle 方式に統一する。

Unit テストで「rebuild_body 内で panic させて running=false に戻ること」を検証する。
...
| 失敗 | 振る舞い |
|---|---|
| Haiku (claude-haiku-4-5) 概念抽出失敗 | 現 iteration abort。**直前までの iteration で成功した concept の wikis は更新済み**（MAX cursor は前進している）。log + メトリクス、state.running=false に戻す |
| 個別 concept の Sonnet (claude-sonnet-4-6) 呼び出し失敗 | その concept のみ wiki 更新せず続行。log + メトリクス。MAX cursor は他の成功 concept で前進するため、当該 concept は **自動再試行されない**。manual `wiki_rebuild` または将来 raw に同 concept tag が乗ったタイミングで再合成（§7.2） |
| `source_refs` に実在しない raw id | その id を除外して保存 |
| worker タスクが panic / 中断 | 親 task で JoinHandle を await し、`JoinError::is_panic()` を捕捉して state.running=false を必ず復元する（§7.3 の正規パターン）。次回 append で復活 |
...
- wiki のフレッシュネス: rebuild が走り終わるまでの間、最新 raw は wiki に未反映。drain loop により同一 worker セッション内では追加 raw を順次取り込むが、staleness window は通常「現セッションの drain が完了するまで」、最大「ユーザが手動 `wiki_rebuild` を呼ぶまで無期限（次の append が来ないケース）」。
- 反映タイミングが要件になる場合、または個別 concept の Sonnet 失敗を検知した場合は、`wiki_rebuild` を明示的に呼ぶ運用とする。
- 観測者向けには `concept_rebuild_failed_total` と `rebuild_drain_iterations` を見ることで状態を判断できる。
...
| `rebuild_drain_iterations` histogram | p99 > 5（drain loop が暴走 / 高頻度 append のサイン） |
| `rebuild_drain_capped_total` 増分 | 5 分で > 1（MAX_ITERATIONS 打ち切り発生＝バックログ恒常化のサイン） |
...
- Manual モード: new_raws が空でも affected concepts を強制再合成すること
- Manual { concept: Some(c) }: Haiku を呼ばずに affected={c} で進むこと
- manual_pending の merge: 既に `None`（全 concept）が pending のとき `Some(c)` が来ても None を維持
- drain MAX_ITERATIONS の打ち切り: 11 回目に入る前に break + 警告ログ
