...
worker は tokio task として spawn される。

```
1. (rebuild 対象 owner で) 前回 rebuild 以降の raws を取得:
...
2. 影響 concepts 特定:
   ...
   - 加えて Haiku (claude-haiku-4-5-20251001) で「新規 raws.content 集合 + 既存 concept 一覧」を入力に、
     影響を受ける既存 concept と新規追加すべき concept を出力
   - 上記の和集合を最終的な影響 concepts

3. 影響 concept ごとに並列 (concurrency 上限 4) で:
...
CASE A: running == false
  → state[owner].running = true
  → worker を spawn (concept 指定があれば affected_concepts として渡す)
  → 戻り: { status: 'started' }

CASE B: running == true
  → state[owner].manual_pending = true
  → 戻り: { status: 'pending' }
     (現 rebuild 完了後、自動的に追加 rebuild が走る)
```

worker は manual_pending を見て trailing 実行する。manual 呼び出しが複数回 burst しても manual_pending は 1 bit なので max 1 回しか trailing しない。

### 7.2 increment cursor の取り扱い

増分起点は **per-owner で `MAX(wikis.last_rebuilt_at)`** を使う（個別 job 履歴を持たないため）。

- 個別 concept の Sonnet 失敗 → その concept の `wikis.last_rebuilt_at` は更新されない → 次回 rebuild で「最古の last_rebuilt_at」が起点になり、その concept は再入力される（自動再試行）
- 結果: 失敗 concept はバックグラウンドで自動的に再挑戦される（冪等寄り）
- 重複コスト懸念があるが MVP は許容。観測メトリクスで失敗率を見る
...
- 中断中に追加された raws は次の rebuild に含まれる
...
| worker タスクが panic / 中断 | tokio が catch_unwind 相当で state.running を強制 false。次回 append で復活 |
