//! Dynamic Client Registration (DCR) / OAuth Client storage for
//! vegapunk-memory-server.
//!
//! llm-memory-storage::oauth_clients と同じ schema・同じ挙動だが、別 SQLite
//! ファイル (vegapunk-memory-server 専用) を見るために物理的に分けてある。
//! 行表現 (`OAuthClient`) と関数のシグネチャは同型。
...
mod tests {
    ...
    async fn register_and_get() {
        ...
        assert_eq!(got.token_endpoint_auth_method, "none");
    }

    #[tokio::test]
    async fn get_returns_none_for_missing_client() {
        let pool = init_pool("sqlite::memory:").await.unwrap();
        assert!(get(&pool, "no-such-client").await.unwrap().is_none());
    }
