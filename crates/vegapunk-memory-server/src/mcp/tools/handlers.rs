...
use vegapunk_client::graphrag::{
    AttributeFilter, Edge, GetSchemaRequest, GetStatsRequest, GetTraceableChainRequest,
    IngestMessage, IngestRawMetadata, IngestRawRequest, IngestRequest, ListSchemasRequest,
    MessageMetadata, Node, NodeAttribute, QueryNodesRequest, SchemaListItem, SearchRequest,
    SearchResultItem, UpsertEdgesRequest, UpsertNodesRequest, UpsertVectorsRequest, VectorEntry,
};
...
// ─── Low-level upsert (UpsertNodes / UpsertEdges / UpsertVectors) ─────────
//
// vegapunk gRPC の `UpsertNodes` / `UpsertEdges` / `UpsertVectors` を MCP tool
// として公開する。`ingest` / `ingest_raw` は vegapunk 側の LLM 抽出を経由する
// ため、同名 entity を含む text を連投すると別 node に分裂する問題がある
// (PR #21 の wrapper-side dedup でも防ぎきれない — 「Vegapunk」と「Vegapunk
// roadmap」のような文脈依存抽出は wrapper の word-boundary scan の射程外)。
//
...
//
// cross-tenant guard:
// vegapunk の `Node` / `Edge` / `VectorEntry` には `schema` フィールドが無く、
// tenant 境界は **id の `{schema}:` prefix** で表現する仕様。wrapper では
// 各 upsert request の id / from_id / to_id を全て `{user.vegapunk_schema}:`
// 接頭辞で始まることを強制し、他 tenant の prefix を持つ id は invalid_args
// で弾く。これで client が `sivira-shared:project-x` のような他 schema 配下
// に書き込もうとしても通らない。

/// `attributes` (= `[{key, value}]` 配列) を `Vec<NodeAttribute>` に変換する。
/// proto 上 `NodeAttribute.value` は string 固定。client が number / bool を
/// 渡したいなら client 側で stringify する責務にする (= wrapper でゆるく
/// coerce すると、後で query_nodes filter (= string 比較) と挙動が食い違う)。
fn build_node_attributes_from(
    obj: &Map<String, Value>,
    owner: &str,
) -> Result<Vec<NodeAttribute>, String> {
    let v = match obj.get("attributes") {
        None | Some(Value::Null) => return Ok(vec![]),
        Some(v) => v,
    };
    let array = v
        .as_array()
        .ok_or_else(|| format!("'{owner}.attributes' must be an array"))?;
    let mut out = Vec::with_capacity(array.len());
    for (i, item) in array.iter().enumerate() {
        let item_owner = format!("{owner}.attributes[{i}]");
        let obj = item
            .as_object()
            .ok_or_else(|| format!("'{item_owner}' must be an object with 'key' and 'value'"))?;
        let key = require_str_field(obj, "key", &item_owner)?;
        // value は **空文字許容** (= attribute を空文字で明示的に設定したいユース
        // ケース、例えば「未設定 → 空文字」の placeholder)。null や非 string は弾く。
        let value = match obj.get("value") {
            None | Some(Value::Null) => {
                return Err(format!("missing required '{item_owner}.value'"));
            }
            Some(v) => v
                .as_str()
                .ok_or_else(|| format!("'{item_owner}.value' must be a string"))?
                .to_string(),
        };
        out.push(NodeAttribute { key, value });
    }
    Ok(out)
}
...
/// 各 `nodes[i].id` は `{user.vegapunk_schema}:` 接頭辞を強制 (cross-tenant guard)。
pub(super) fn build_upsert_nodes_request(
    user_schema: &str,
    args: &Value,
) -> Result<UpsertNodesRequest, String> {
    let args = require_object_args(args)?;
    let nodes_v = args
        .get("nodes")
        .ok_or_else(|| "missing required argument: 'nodes'".to_string())?;
    ...
    for (i, item) in nodes_arr.iter().enumerate() {
        ...
        let obj = item
            .as_object()
            .ok_or_else(|| format!("'{owner}' must be an object"))?;
        let id = require_tenant_scoped_id(obj, "id", &owner, user_schema)?;
        let r#type = require_str_field(obj, "type", &owner)?;
        let attributes = build_node_attributes_from(obj, &owner)?;
        nodes.push(Node {
            id,
            r#type,
            attributes,
...
pub(super) async fn upsert_nodes(state: &AppState, user: &AuthenticatedUser, args: Value) -> Value {
    ...
    let mut client = state.vegapunk.clone();
    match client.upsert_nodes(req).await {
        Err(status) => tonic_error_content("UpsertNodes", status),
        Ok(resp) => {
            let r = resp.into_inner();
            success_content(json!({ "upserted_count": r.upserted_count }))
        }
    }
}

/// `upsert_edges` arguments を `UpsertEdgesRequest` に詰める。
/// `from_id` / `to_id` の両方を `{user_schema}:` 接頭辞で強制。
pub(super) fn build_upsert_edges_request(
    user_schema: &str,
    args: &Value,
) -> Result<UpsertEdgesRequest, String> {
    let args = require_object_args(args)?;
    let edges_v = args
        .get("edges")
        .ok_or_else(|| "missing required argument: 'edges'".to_string())?;
    let edges_arr = edges_v
        .as_array()
    ...
    for (i, item) in edges_arr.iter().enumerate() {
        let owner = format!("edges[{i}]");
        let obj = item
            .as_object()
            .ok_or_else(|| format!("'{owner}' must be an object"))?;
        let from_id = require_tenant_scoped_id(obj, "from_id", &owner, user_schema)?;
        let to_id = require_tenant_scoped_id(obj, "to_id", &owner, user_schema)?;
        let r#type = require_str_field(obj, "type", &owner)?;
        let attributes = build_node_attributes_from(obj, &owner)?;
        edges.push(Edge {
            from_id,
...
pub(super) async fn upsert_edges(state: &AppState, user: &AuthenticatedUser, args: Value) -> Value {
    ...
    let mut client = state.vegapunk.clone();
    match client.upsert_edges(req).await {
        Err(status) => tonic_error_content("UpsertEdges", status),
        Ok(resp) => {
            let r = resp.into_inner();
            success_content(json!({ "upserted_count": r.upserted_count }))
        }
...
/// `upsert_vectors` arguments を `UpsertVectorsRequest` に詰める。
/// `id` は `{user_schema}:` 接頭辞強制。`vector` は非空の number 配列。
/// `metadata` は string→string の map (proto の制約)。
pub(super) fn build_upsert_vectors_request(
    user_schema: &str,
    args: &Value,
) -> Result<UpsertVectorsRequest, String> {
    let args = require_object_args(args)?;
    let vecs_v = args
        .get("vectors")
        .ok_or_else(|| "missing required argument: 'vectors'".to_string())?;
    let vecs_arr = vecs_v
        .as_array()
        .ok_or_else(|| "'vectors' must be an array".to_string())?;
    ...
    for (i, item) in vecs_arr.iter().enumerate() {
        ...
        let vec_v = obj
            .get("vector")
            .ok_or_else(|| format!("missing required '{owner}.vector'"))?;
        let vec_arr = vec_v
            .as_array()
        ...
        let metadata = match obj.get("metadata") {
            ...
            Some(v) => {
                let mobj = v
                    .as_object()
                    .ok_or_else(|| format!("'{owner}.metadata' must be an object"))?;
                let mut m = HashMap::with_capacity(mobj.len());
                for (k, val) in mobj {
                    let s = val.as_str().ok_or_else(|| {
                        format!("'{owner}.metadata.{k}' must be a string (proto制約)")
                    })?;
...
pub(super) async fn upsert_vectors(
    state: &AppState,
    user: &AuthenticatedUser,
    args: Value,
) -> Value {
    ...
    let mut client = state.vegapunk.clone();
    match client.upsert_vectors(req).await {
        Err(status) => tonic_error_content("UpsertVectors", status),
        Ok(resp) => {
            let r = resp.into_inner();
            success_content(json!({ "upserted_count": r.upserted_count }))
        }
...
mod tests {
    ...
    // ── upsert_nodes / upsert_edges / upsert_vectors ─────────────────

    #[test]
    fn upsert_nodes_request_strips_to_proto_and_keeps_attributes() {
        let req = build_upsert_nodes_request(
            "user-alice",
            &json!({
                "nodes": [{
                    "id": "user-alice:proj-vegapunk",
                    "type": "Project",
                    "attributes": [
                        {"key": "name", "value": "Vegapunk"},
                        {"key": "summary", "value": "GraphRAG engine"}
                    ]
                }]
            }),
        )
        ...
        let n = &req.nodes[0];
        assert_eq!(n.id, "user-alice:proj-vegapunk");
        assert_eq!(n.r#type, "Project");
        assert_eq!(n.attributes.len(), 2);
        assert_eq!(n.attributes[0].key, "name");
        assert_eq!(n.attributes[0].value, "Vegapunk");
        assert_eq!(n.attributes[1].key, "summary");
    }

    #[test]
    fn upsert_nodes_request_rejects_cross_tenant_id() {
        // bob のスキーマの id を alice 経由で投入しようとする = guard 発火。
        ...
        assert!(err.contains("cross-tenant"), "guard label in error: {err}");
    }

    #[test]
    fn upsert_nodes_request_rejects_shared_prefix_when_user_is_personal() {
        // sivira-shared の id は personal tenant が書き込めない。
        // (PR #21 で「shared 書き込み禁止」を user が明示)
        let err = build_upsert_nodes_request(
            "user-alice",
            &json!({
                "nodes": [{
                    "id": "sivira-shared:proj-x",
                    "type": "Project"
                }]
            }),
        )
        .unwrap_err();
        assert!(err.contains("user-alice:"), "got: {err}");
    }
    ...
    #[test]
    fn upsert_nodes_request_attributes_default_to_empty() {
        let req = build_upsert_nodes_request(
            "user-alice",
            &json!({
                "nodes": [{
        ...
        .unwrap();
        assert!(req.nodes[0].attributes.is_empty());
    }

    #[test]
    fn upsert_nodes_request_rejects_non_string_attribute_value() {
        // proto は string only。number / null を coerce しない。
        let err = build_upsert_nodes_request(
            "user-alice",
            &json!({
                "nodes": [{
                    "id": "user-alice:proj-x",
                    "type": "Project",
                    "attributes": [{"key": "count", "value": 42}]
                }]
            }),
        )
        .unwrap_err();
        assert!(err.contains("attributes[0].value"), "got: {err}");
    }

    #[test]
    fn upsert_edges_request_strips_to_proto() {
        let req = build_upsert_edges_request(
            "user-alice",
            &json!({
                "edges": [{
                    "from_id": "user-alice:proj-a",
                    "to_id": "user-alice:proj-b",
                    "type": "depends_on"
                }]
            }),
        )
        ...
        assert_eq!(req.edges[0].from_id, "user-alice:proj-a");
        assert_eq!(req.edges[0].to_id, "user-alice:proj-b");
        assert_eq!(req.edges[0].r#type, "depends_on");
    }

    #[test]
    fn upsert_edges_request_rejects_cross_tenant_from() {
        let err = build_upsert_edges_request(
            "user-alice",
            &json!({
                "edges": [{
                    "from_id": "user-bob:proj-a",
                    "to_id": "user-alice:proj-b",
                    "type": "rel"
                }]
            }),
        ...
        .unwrap_err();
        assert!(err.contains("edges[0].from_id"), "got: {err}");
    }

    #[test]
    fn upsert_edges_request_rejects_cross_tenant_to() {
        let err = build_upsert_edges_request(
            "user-alice",
            &json!({
                "edges": [{
                    "from_id": "user-alice:proj-a",
                    "to_id": "user-bob:proj-b",
                    "type": "rel"
                }]
            }),
        )
        .unwrap_err();
        assert!(err.contains("edges[0].to_id"), "got: {err}");
    }

    #[test]
    fn upsert_vectors_request_converts_floats_and_metadata() {
        let req = build_upsert_vectors_request(
            "user-alice",
            &json!({
                "vectors": [{
                    "id": "user-alice:proj-x",
        ...
        let v = &req.vectors[0];
        assert_eq!(v.id, "user-alice:proj-x");
        assert_eq!(v.vector.len(), 3);
    ...
    #[test]
    fn upsert_vectors_request_rejects_empty_vector() {
    ...
    #[test]
    fn upsert_vectors_request_rejects_non_string_metadata_value() {
        let err = build_upsert_vectors_request(
            "user-alice",
            &json!({
                "vectors": [{
        ...
        .unwrap_err();
        assert!(err.contains("metadata.dim"), "got: {err}");
    }

    #[test]
    fn upsert_vectors_request_rejects_cross_tenant_id() {
        let err = build_upsert_vectors_request(
            "user-alice",
            &json!({
                "vectors": [{
        ...
        .unwrap_err();
        assert!(err.contains("vectors[0].id"), "got: {err}");
    }
