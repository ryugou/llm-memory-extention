...
pub async fn get(
    ...
    owner_id: &str,
    concept: &str,
) -> Result<Option<Wiki>, StorageError> {
...
pub async fn max_last_rebuilt_at(
    pool: &SqlitePool,
    scope: Scope,
    owner_id: &str,
) -> Result<i64, StorageError> {
    let (v,): (Option<i64>,) =
        sqlx::query_as("SELECT MAX(last_rebuilt_at) FROM wikis WHERE scope = ? AND owner_id = ?")
            .bind(scope.as_str())
            .bind(owner_id)
            .fetch_one(pool)
            .await?;
...
pub async fn count_concepts(
    pool: &SqlitePool,
    scope: Scope,
    owner_id: &str,
) -> Result<i64, StorageError> {
    let (n,): (i64,) =
        sqlx::query_as("SELECT COUNT(*) FROM wikis WHERE scope = ? AND owner_id = ?")
            .bind(scope.as_str())
            .bind(owner_id)
            .fetch_one(pool)
            .await?;
...
mod tests {
    ...
    async fn upsert_replaces_existing() {
        ...
        upsert(&pool, Scope::Personal, "u1", "concept-a", "v1", "[]", 100)
            .await
            .unwrap();
        upsert(&pool, Scope::Personal, "u1", "concept-a", "v2", "[]", 200)
            .await
            .unwrap();
        let w = get(&pool, Scope::Personal, "u1", "concept-a")
            .await
            .unwrap()
            .unwrap();
    ...
    async fn max_last_rebuilt_at_returns_zero_when_empty() {
        let pool = init_pool("sqlite::memory:").await.unwrap();
        let v = max_last_rebuilt_at(&pool, Scope::Personal, "u1")
            .await
            .unwrap();
        assert_eq!(v, 0);
    }
    ...
    async fn list_concepts_alphabetical() {
        let pool = init_pool("sqlite::memory:").await.unwrap();
        upsert(&pool, Scope::Personal, "u1", "zeta", "x", "[]", 1)
            .await
            .unwrap();
        upsert(&pool, Scope::Personal, "u1", "alpha", "x", "[]", 1)
            .await
            .unwrap();
        assert_eq!(
            list_concepts(&pool, Scope::Personal, "u1").await.unwrap(),
            vec!["alpha", "zeta"]
        );
    }
    ...
    async fn count_concepts_reflects_inserts() {
        let pool = init_pool("sqlite::memory:").await.unwrap();
        assert_eq!(
            count_concepts(&pool, Scope::Personal, "u1").await.unwrap(),
            0
        );
        upsert(&pool, Scope::Personal, "u1", "c1", "x", "[]", 1)
            .await
            .unwrap();
        upsert(&pool, Scope::Personal, "u1", "c2", "x", "[]", 1)
            .await
            .unwrap();
        assert_eq!(
            count_concepts(&pool, Scope::Personal, "u1").await.unwrap(),
    ...
    async fn owner_isolation_in_personal_scope() {
        ...
        .await
        .unwrap();
        let u1 = get(&pool, Scope::Personal, "u1", "shared-name")
            .await
            .unwrap()
            .unwrap();
        let u2 = get(&pool, Scope::Personal, "u2", "shared-name")
            .await
            .unwrap()
            .unwrap();
