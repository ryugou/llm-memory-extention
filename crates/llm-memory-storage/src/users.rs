...
pub async fn delete_cascade(pool: &SqlitePool, user_id: &str) -> Result<(), StorageError> {
    ...
    sqlx::query("DELETE FROM raws WHERE scope='personal' AND owner_id = ?")
        .bind(user_id)
        .execute(&mut *tx)
        .await?;
    sqlx::query("DELETE FROM wikis WHERE scope='personal' AND owner_id = ?")
        .bind(user_id)
        .execute(&mut *tx)
        .await?;
    sqlx::query("DELETE FROM schemas WHERE scope='personal' AND owner_id = ?")
        .bind(user_id)
        .execute(&mut *tx)
        .await?;
...
mod tests {
    ...
    async fn upsert_is_idempotent_on_provider_subject() {
        let pool = init_pool("sqlite::memory:").await.unwrap();
        let u1 = upsert(&pool, "01HJ1", "google", "sub-x", Some("old@example.com"))
            .await
            .unwrap();
        let u2 = upsert(&pool, "01HJ2", "google", "sub-x", Some("new@example.com"))
            .await
            .unwrap();
        assert_eq!(
            u1.id, u2.id,
            "same provider+subject should map to same user"
        );
