...
async fn register(State(state): State<AsState>, Json(body): Json<dcr::DcrRequest>) -> Response {
    ...
    let client = match vegapunk_memory_storage::oauth_clients::register(
        &state.pool,
        &redirect_uris_json,
        &grant_types_json,
...
async fn grant_auth_code(state: AsState, body: TokenForm) -> Response {
    ...
    // RFC 6749 §4.1.3 / RFC 8252 で、public client (token_endpoint_auth_method=none)
    // の authorization_code grant は client_id と redirect_uri を必須にして、
    // /oauth/authorize で発行された code が「どの client / どの redirect 経路で
    // 発行されたか」を token endpoint で再確認させる必要がある。
    // 省略を許すと、code を奪取した別 actor が異なる client_id で code を redeem
    // して token を取れる mix-up 攻撃を許してしまう。
    let req_client = match body.client_id {
        Some(c) => c,
        None => return bad_request("invalid_request", "client_id is required"),
    };
    if req_client != entry.client_id {
        return bad_request("invalid_grant", "client_id mismatch");
    }
    let req_uri = match body.redirect_uri {
        Some(u) => u,
        None => return bad_request("invalid_request", "redirect_uri is required"),
    };
...
async fn grant_refresh(state: AsState, body: TokenForm) -> Response {
    ...
    let (user_id, client_id) =
        match vegapunk_memory_storage::tokens::validate_and_revoke(&state.pool, &tok, now).await {
            Ok(Some(x)) => x,
...
async fn issue_tokens(
    ...
    let refresh = if include_refresh {
        ...
        if let Err(e) = vegapunk_memory_storage::tokens::create_refresh(
            &state.pool,
            &r,
            user_id,
            client_id,
...
async fn revoke(State(state): State<AsState>, Form(body): Form<RevokeForm>) -> impl IntoResponse {
    ...
    let _ = vegapunk_memory_storage::tokens::revoke(&state.pool, &body.token, now_ms()).await;
    StatusCode::OK
}
