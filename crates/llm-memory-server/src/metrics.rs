...
impl Metrics {
    pub fn new() -> Self {
        ...
        let rebuild_in_flight =
            IntGauge::new("rebuild_in_flight_gauge", "rebuild in-flight workers").unwrap();
        let rebuild_duration = Histogram::with_opts(
            HistogramOpts::new("rebuild_duration_seconds", "rebuild iteration duration").buckets(
        ...
        let concept_rebuild_failed =
            IntCounter::new("concept_rebuild_failed_total", "per-concept failures").unwrap();
        ...
        let dcr_registration =
            IntCounter::new("dcr_registration_total", "dcr registrations").unwrap();
        ...
        registry
            .register(Box::new(rebuild_in_flight.clone()))
            .unwrap();
        registry
            .register(Box::new(rebuild_duration.clone()))
            .unwrap();
        registry
            .register(Box::new(rebuild_drain_iterations.clone()))
            .unwrap();
        registry
