...
mod tests {
    ...
    async fn limit_is_enforced() {
        ...
        for i in 0..60 {
            insert(
                ...
                NewRaw {
                    ...
                    owner_id: "u1",
                    title: &format!("vegapunk-{i}"),
                    content: "graphrag knowledge",
                    source: "m",
                    tags_json: None,
                    created_by: Some("u1"),
                },
        ...
        let out = build(&pool, Scope::Personal, "u1", "vegapunk", &[], &[])
            .await
            .unwrap();
    ...
    async fn existing_source_refs_come_first() {
        ...
        .await
        .unwrap();
        let out = build(&pool, Scope::Personal, "u1", "alpha", &[r.id.clone()], &[])
            .await
            .unwrap();
